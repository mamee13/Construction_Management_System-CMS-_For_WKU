...
/* eslint-disable */
import { useState, useEffect } from "react";
import { useNavigate, useParams } from "react-router-dom"; // Link is not used here, removed
import { useQuery } from "@tanstack/react-query";
import { toast } from "react-toastify";
...
import {
  ...
  XCircleIcon,
  ArrowDownTrayIcon, // Icon for PDF download button
  PhotoIcon,         // Icon for image attachments
  DocumentTextIcon,  // Icon for PDF attachments
                ...
                {onRetry && (
                    <div className="mt-4">
                        <button
                            ...
                            className="inline-flex items-center px-3 py-1.5 border border-transparent text-sm font-medium rounded-md text-red-700 bg-red-100 hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
                        >
                             <ArrowPathIcon className="h-4 w-4 mr-1.5"/> {/* Added icon */}
                            Try again
                        </button>
                    </div>
                )}
...
// --- Helper Function for Attachments (Copied from other components) ---
...
const ReportDetail = () => {
  ...
  const {
    ...
    queryKey: ["report", id, "consultant"], // Added role to key for potential caching difference
    queryFn: () => reportsAPI.getReportById(id),
    enabled: !!id && isConsultant && !isLoadingUser,
    staleTime: 5 * 60 * 1000,
  });

  // --- Loading States ---
  if (isLoadingUser) {
      return <div className="py-10 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto"><LoadingSpinner message="Checking user permissions..." /></div>;
  }
   if (!isConsultant) return null; // Should have navigated away
  if (isLoadingReport) {
  ...
  if (reportError) {
    return (
      <div className="py-10 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
        <ErrorDisplay error={reportError} onRetry={refetch} />
        <div className="mt-6 text-center">
            <button type="button" onClick={() => navigate("/reports")} className="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
  ...
  const isOwnReport = report.generatedBy?._id === currentUser?._id || report.generatedBy === currentUser?._id;
  const canEdit = isOwnReport && ['draft', 'pending', 'rejected'].includes(report.status); // Define editability
  ...
  return (
    <div className="py-10 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
      ...
      <div className="sm:flex sm:items-center sm:justify-between mb-8">
        ...
        <div className="mt-4 sm:mt-0 sm:ml-4 flex-shrink-0 flex flex-wrap gap-2 justify-end"> {/* Use flex-wrap and gap */}
            ...
            >
              {({ loading, error }) => (
                <button
                  ...
                  onClick={() => { // Log error on click if exists
                      if (error) {
                          console.error("PDF Generation Error (Consultant View):", error);
          ...
          {canEdit && (
            <button
              type="button"
              onClick={() => navigate(`/reports/edit/${report._id}`)} // Assuming consultant edit route is /reports/edit/:id
              className="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
              title="Edit this report"
            >
          ...
          <button
            type="button"
            onClick={() => navigate("/reports")} // Navigate to consultant reports list
            className="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
          >
            <ArrowLeftIcon className="h-5 w-5 mr-2" /> Back to Reports
          </button>
      ...
      <div className="bg-white shadow overflow-hidden sm:rounded-lg">
        <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
           ...
           <div className="flex items-start justify-between">
                <div>
                    <h3 className="text-lg leading-6 font-medium text-gray-900">Report Details</h3>
                    <p className="mt-1 max-w-2xl text-sm text-gray-500">Generated on {reportsAPI.formatDate(report.generatedAt || report.createdAt)}</p>
                </div>
        ...
        <div className="px-4 py-5 sm:p-0">
          <dl className="sm:divide-y sm:divide-gray-200">
            {/* Basic Info */}
            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"><dt className="text-sm font-medium text-gray-500">Project</dt><dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{report.project?.projectName ? ( <span className="hover:text-indigo-600">{report.project.projectName}</span> ) : ( <span className="text-gray-400 italic">Unknown Project</span> )}</dd></div>
            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"><dt className="text-sm font-medium text-gray-500">Report Type</dt><dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{reportsAPI.getReportTypeLabel(report.type)}</dd></div>
            {(report.periodStartDate || report.periodEndDate) && (<div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"><dt className="text-sm font-medium text-gray-500">Reporting Period</dt><dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{report.periodStartDate ? reportsAPI.formatDate(report.periodStartDate) : "N/A"} to {report.periodEndDate ? reportsAPI.formatDate(report.periodEndDate) : "N/A"}</dd></div>)}
            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"><dt className="text-sm font-medium text-gray-500">Generated By</dt><dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{isOwnReport ? "You" : (report.generatedBy?.firstName ? `${report.generatedBy.firstName} ${report.generatedBy.lastName}` : (report.generatedBy?.username || report.generatedBy || <span className="text-gray-400 italic">Unknown Consultant</span>))}</dd></div>
            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"><dt className="text-sm font-medium text-gray-500">Last Updated</dt><dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{reportsAPI.formatDate(report.updatedAt, { year: "numeric", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit", hour12: true })}</dd></div>

            {/* Key Metrics */}
            {report.keyMetrics && Object.keys(report.keyMetrics).length > 0 && (<div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"><dt className="text-sm font-medium text-gray-500">Key Metrics</dt><dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"><div className="grid grid-cols-1 gap-4 sm:grid-cols-2">{Object.entries(report.keyMetrics).filter(([_, value]) => value !== null && value !== undefined).map(([key, value]) => (<div key={key} className="bg-gray-50 p-4 rounded-lg border border-gray-200"><p className="text-sm font-medium text-gray-600 truncate capitalize">{key.replace(/([A-Z])/g, ' $1').replace(/_/g, ' ')}</p><p className="mt-1 text-2xl font-semibold text-gray-900">{typeof value === 'number' && key.toLowerCase().includes('percentage') ? `${value.toFixed(1)}%` : value.toString()}</p></div>))}</div></dd></div>)}

            {/* Summary */}
            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"><dt className="text-sm font-medium text-gray-500">Summary</dt><dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 whitespace-pre-wrap break-words">{report.summary || <span className="text-gray-400 italic">No summary provided.</span>}</dd></div>

            {/* Issues and Risks */}
            {report.issuesAndRisks && report.issuesAndRisks.length > 0 && (<div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"><dt className="text-sm font-medium text-gray-500">Issues and Risks</dt><dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"><ul className="border border-gray-200 rounded-md divide-y divide-gray-200">{report.issuesAndRisks.map((issue, index) => (<li key={index} className="px-4 py-3 flex items-start space-x-3"><span className={`mt-0.5 px-2 py-0.5 inline-flex text-xs leading-4 font-semibold rounded-full whitespace-nowrap ${ issue.severity === 'high' ? 'bg-red-100 text-red-800' : issue.severity === 'medium' ? 'bg-yellow-100 text-yellow-800' : issue.severity === 'low' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }`}>{reportsAPI.getReportStatusLabel(issue.severity || 'medium')}</span><div className="flex-1 min-w-0"><p className="text-sm text-gray-800">{issue.description}</p>{issue.reportedAt && (<p className="text-xs text-gray-500">Reported: {reportsAPI.formatDate(issue.reportedAt, { month: 'short', day: 'numeric' })}</p>)}</div></li>))}</ul></dd></div>)}

            {/* Attachments (Added Download Button) */}
            {report.attachments && report.attachments.length > 0 && (
                <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    <dt className="text-sm font-medium text-gray-500">Attachments</dt>
                    <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                    ...
                                    <li key={index} className="pl-3 pr-4 py-3 flex items-center justify-between text-sm space-x-3">
                                        <div className="w-0 flex-1 flex items-center min-w-0">
                                            ...
                                            <span className="ml-2 flex-1 w-0 truncate font-medium">
                                                {file.url ? (
                                                    <a href={file.url} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:text-indigo-500 hover:underline" title={`Open ${file.fileName || 'attachment'}`}>
                                                        {file.fileName || 'Attached File'}
                                                    </a>
                                                ) : (
                                                    <span className="text-gray-700">{file.fileName || 'Attached File (No Link)'}</span>
                                                )}
                                            </span>
                                        </div>
            ...
            {/* Feedback */}
            {report.feedback && (<div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"><dt className="text-sm font-medium text-gray-500">Admin Feedback</dt><dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"><div className="bg-yellow-50 border border-yellow-200 p-4 rounded-md whitespace-pre-wrap break-words">{report.feedback}</div></dd></div>)}
          </dl>
        </div>
      </div>
    </div>
