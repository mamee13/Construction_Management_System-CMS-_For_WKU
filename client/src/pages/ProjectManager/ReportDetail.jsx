...
import {
  ...
  PhotoIcon,         // Icon for image attachments (needed for getFileType)
} from "@heroicons/react/24/outline"
import authAPI from "../../api/auth"
import reportsAPI from "../../api/reports"
// Remove unused import if ReportDetail is not used within this component
// import ReportDetail from "../Consultant/reports/ReportDetail";
// Import the separate PDF document structure component
// Ensure this path is correct!
import ReportPdfDocument from '../../utils/ReportPdfDocument'; // Assuming it's in the same folder
...
// Removed the specific parseIssuesAndRisks debug function as it's handled differently now


const ReportDetailForProjectManager = () => {
  const { id } = useParams()
  const navigate = useNavigate()
  const [isProjectManager, setIsProjectManager] = useState(false)
  const [isLoadingUser, setIsLoadingUser] = useState(true)
  // Removed local state for issuesAndRisks as it comes directly from query data
  // const [issuesAndRisks, setIssuesAndRisks] = useState([])
  const currentUser = authAPI.getCurrentUser()

  // --- Effect for Role Check ---
  useEffect(() => {
    // console.log("[FE PM Detail] Checking user role...")
    if (currentUser) {
      setIsLoadingUser(false)
      const userRole = currentUser.role
      // console.log("[FE PM Detail] User Role:", userRole)
      const isUserPM = userRole === "project_manager" || userRole === "admin" // Allow Admin too
      setIsProjectManager(isUserPM)

      if (!isUserPM) {
        // console.warn("[FE PM Detail] Access denied. Not a project manager or admin.")
        toast.warn("Access denied. This page is for project managers and admins only.", { autoClose: 3000 })
        navigate("/dashboard")
      } else {
        // console.log("[FE PM Detail] User is a project manager or admin. Proceeding.")
      }
    } else {
      setIsLoadingUser(false)
      // console.error("[FE PM Detail] Authentication required.")
  ...
  const {
    ...
    queryKey: ["report", id, "project-manager"], // Use specific key
    queryFn: () => {
    //   console.log(`[FE PM Detail] Fetching report with ID: ${id}`)
      return reportsAPI.getReportById(id)
    },
    enabled: !!id && !isLoadingUser && isProjectManager,
    staleTime: 5 * 60 * 1000,
    retry: 1,
    // onSuccess removed, data parsing happens during render if needed
  })
  ...
  if (!isProjectManager) return null; // Should have navigated away
  if (isLoadingReport) {
  ...
  // --- Error State ---
  if (isReportError) {
    // console.error("[FE PM Detail] Error loading report:", reportError)
    return (
      <div className="py-10 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
        ...
        <div className="mt-6 text-center">
          <button type="button" onClick={() => navigate("/project-manager/reports")} className="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
  ...
  // console.log("[FE PM Detail] Report data received:", report ? JSON.stringify(report).substring(0, 300) + "..." : "No data");

  if (!report) {
    // console.warn("[FE PM Detail] Report data is missing after successful fetch.")
    return (
      <div className="py-10 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
  ...
  const isOwnReport = reportUser?._id === currentUser?._id // PM viewing own report? Unlikely but possible
  // Allow PM/Admin to edit if *they* generated it and status is draft/pending
  const canEdit = isOwnReport && ['draft', 'pending'].includes(report.status);
  const showFeedback = Boolean(report.feedback)
  // Ensure issuesAndRisks is an array for mapping, handle potential string data if needed
  const issuesAndRisks = Array.isArray(report.issuesAndRisks) ? report.issuesAndRisks : [];
    ...
    <div className="py-10 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
      ...
      <div className="sm:flex sm:items-center sm:justify-between mb-8">
        ...
        <div className="mt-4 sm:mt-0 sm:ml-4 flex-shrink-0 flex flex-wrap gap-2 justify-end"> {/* Use flex-wrap and gap */}
                ...
                <button
                  ...
                  title={error ? `Error generating PDF: ${error}`: (loading ? 'Generating PDF...' : 'Download report as PDF')}
                   onClick={() => { // Log error on click if exists
                      if (error) {
                          console.error("PDF Generation Error (PM View):", error);
          ...
          {canEdit && (
            <Link
              // Ensure the edit route is correct for PM/Admin editing their own report
              to={`/project-manager/reports/edit/${report._id}`} // Adjust route if needed
          ...
          <button
            type="button"
            onClick={() => navigate("/project-manager/reports")} // Navigate to PM reports list
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm text-gray-500">
                Submitted on {reportsAPI.formatDate(report.createdAt)}
              </p>
            </div>
            {report.status && (
              <span className={`ml-3 px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${reportsAPI.getReportStatusColor(report.status)}`}>
        ...
        <div className="px-4 py-5 sm:p-0">
          <dl className="sm:divide-y sm:divide-gray-200">
            ...
            <div className="py-3 sm:py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"><dt className="text-sm font-medium text-gray-500 flex items-center"><DocumentTextIcon className="h-4 w-4 mr-1.5 text-gray-400"/>Summary</dt><dd className="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-2 whitespace-pre-wrap break-words">{report.summary || <span className="text-gray-400 italic">No summary provided.</span>}</dd></div>
             ...
             {/* Issues and Risks (Using derived 'issuesAndRisks' array) */}
             {issuesAndRisks.length > 0 && (
                 <div className="py-3 sm:py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                     <dt className="text-sm font-medium text-gray-500 flex items-center"><ExclamationTriangleIcon className="h-4 w-4 mr-1.5 text-yellow-500"/>Issues & Risks</dt>
                     <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                         <ul className="border border-gray-200 rounded-md divide-y divide-gray-200">
                             {issuesAndRisks.map((issue, index) => (
                                 <li key={index} className="px-3 py-2 flex items-start space-x-3">
             ...
             {/* Handle case where there might be raw data but it's not an array */}
             {!issuesAndRisks.length && report.issuesAndRisks && (
                <div className="py-3 sm:py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    <dt className="text-sm font-medium text-gray-500 flex items-center"><ExclamationTriangleIcon className="h-4 w-4 mr-1.5 text-yellow-500"/>Issues & Risks</dt>
                    <dd className="mt-1 text-sm text-gray-500 italic sm:mt-0 sm:col-span-2">No valid issues or risks reported.</dd>
                </div>
             )}
                ...
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                  <ul role="list" className="border border-gray-200 rounded-md divide-y divide-gray-200">
                    {report.attachments.map((file, index) => {
                        const fileType = getFileType(file.fileName); // Use helper
                        return (
                            <li key={index} className="pl-3 pr-4 py-2 flex items-center justify-between text-sm space-x-3">
                                <div className="w-0 flex-1 flex items-center min-w-0">
                                    ...
                                    <span className="ml-2 flex-1 w-0 truncate font-medium">
                                        {file.url ? (
                                            <a href={file.url} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:text-indigo-500 hover:underline" title={`View ${file.fileName || 'file'}`}>
            ...
            {/* Admin Feedback */}
            {showFeedback && (
              <div className="py-3 sm:py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 bg-red-50/50">
                <dt className="text-sm font-medium text-red-600 flex items-center"><ExclamationTriangleIcon className="h-4 w-4 mr-1.5 text-red-400"/>Admin Feedback</dt>
                <dd className="mt-1 text-sm text-gray-800 sm:mt-0 sm:col-span-2"><div className="border border-red-200 p-3 rounded-md whitespace-pre-wrap break-words bg-white shadow-sm">{report.feedback}</div></dd>
              </div>
            )}
          </dl>
        </div>
...
export default ReportDetailForProjectManager